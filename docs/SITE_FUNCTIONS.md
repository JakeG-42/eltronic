...
- Resend delivery uses the official `resend` package and requires `RESEND_API_KEY` plus `CONTACT_NOTIFICATION_FROM`. A temporary direct SMTP mode can be enabled with `CONTACT_NOTIFICATION_TRANSPORT=direct`; it sends from the Vercel function to recipient MX servers and may be rejected or spammed without retries. The temporary live recipient is `jakubgajosz1999@gmail.com` because `onboarding@resend.dev` can send only to the Resend account email until an Eltronic domain is verified. The onboarding test and a live enquiry notification test both completed, though the onboarding email landed in spam. Launch email should use a verified Eltronic domain sender and the preferred owner inbox. `npm run email:check -- --send --onboarding` sends the Resend first-email test to the account email.
