...
- Resend delivery uses the official `resend` package and requires `RESEND_API_KEY` plus `CONTACT_NOTIFICATION_FROM`. Production is currently testing temporary direct SMTP mode with `CONTACT_NOTIFICATION_TRANSPORT=direct`; it sends from the Vercel function to recipient MX servers and may be rejected or spammed without retries. The current live recipient is `jakubgajosz1999@gmail.com`; a live direct SMTP enquiry test completed without immediate Vercel warning, but final inbox/spam delivery still needs Jake's confirmation. Launch email should use a verified Eltronic domain sender and the preferred owner inbox. `npm run email:check -- --send --onboarding` sends the Resend first-email test to the account email.
