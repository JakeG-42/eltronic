...
- `src/app/studio/(admin)/media/page.tsx`: product media overview derived from managed product galleries.
...
Product gallery editing in Studio uses `src/components/studio/product-image-manager.tsx`, which posts an ordered `galleryImagesJson` payload plus repeated `imageSrc`, `imageAlt` and `imageFileName` fields for compatibility. Each gallery row supports either a typed/pasted URL or a local upload button; uploaded raster files are resized in the browser and stored as inline image data in the managed product record, with a normalized `fileName` stored so Studio does not display the raw data URL. Product galleries are intentionally open-ended: do not add a template-specific image cap. Product galleries should be managed through seed data, Studio image fields or the generated-gallery manifest only. `productFromFormData()` in `src/lib/managed-data.ts` keeps backward compatibility with legacy newline `images` form data, but the UI should stay visual rather than returning to textarea-based image ordering.
...
- Images: visual Studio manager posts `galleryImagesJson` and repeated `imageSrc`, `imageAlt` and `imageFileName` fields.
...
- `getProductImages(product)` returns managed media only: ordered `product.images` first, or the primary `product.image` if no gallery images are set. It should return every valid managed image, not a sliced/template-limited subset.
